...
(() => {
    const app = Sammy('#rooter', function () {
        ...
        this.post('#/shareRecipe', function (ctx) {
            ...
            if (meal.length < 4) {
                ctx.error = 'Meal should be at least 4 characters long!';
                loadPage(ctx, './templates/shareRecD:\Downloads\PU\CookUni_Resources\templates\shareRecipe.hbsipe.hbs');
            }
            else if (ingredients.length < 2) {
                ctx.error = 'Ingredients should be at least 2 elements long!';
                loadPage(ctx, './templates/shD:\Downloads\PU\CookUni_Resources\templates\shareRecipe.hbsareRecipe.hbs');
            }
            else if (prepMethod.length < 10 || description.length < 10) {
                ctx.error = 'The preparation method and description should be at least 10 characters long each!';
                loadPage(ctx, './templates/shD:\Downloads\PU\CookUni_Resources\templates\shareRecipe.hbsareRecipe.hbs');
                ...
                ctx.error = 'You need to select category!';
                loadPage(ctx, './templates/shaD:\Downloads\PU\CookUni_Resources\templates\shareRecipe.hbsreRecipe.hbs');
            }
            else {
                const obj = {
                    likesCounter,
