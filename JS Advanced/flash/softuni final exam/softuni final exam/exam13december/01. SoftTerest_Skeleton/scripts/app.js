...
(() => {
    const app = Sammy('#js', function () {
        ...
        this.get('#/login', function (ctx) {
            loadPage(ctx, './templates/login.hbs')
        });
        this.get('#/register', function (ctx) {
            loadPage(ctx, './templates/register.hbs')
        });
        ...
        this.post('#/register', function (ctx) {
            ...
            if (username.length < 3) {
                ...
                notifications.error().innerHTML = 'Your password does not match!';
                notifications.error().style.display = 'block';
            }
            else {
                notifications.error().style.display = 'none';
